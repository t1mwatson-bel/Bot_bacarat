...
sys.stdout.flush()
print("=" * 60, flush=True)
print("🃏 ПАРСЕР 21 КЛАССИК - V3 API (ЦВЕТНЫЕ МАСТИ)", flush=True)
print("=" * 60, flush=True)
...
def get_active_game_id():
    """Получает ID активной игры через V3 API (21 Классик)"""
    try:
...
def get_game_data(game_id):
    """Получает данные игры через V3 API (без GetGameZip)"""
    try:
        url = "https://1xlite-84484.pro/service-api/main-live-feed/v3/games1x2?cfView=3&count=40&fcountry=1&gr=415&grMode=4&lng=ru&ref=7&selectedMs=1.146.2092323,2.146.2092323,10.146.2092323"
        response = requests.get(url, headers=HEADERS, timeout=10)
...
def format_cards_hand(cards):
    """Форматирует карты с цветом для HTML"""
    if not cards:
        return ""
    result = []
    for c in cards:
...
def format_cards_raw(cards):
    """Форматирует карты без цвета для внутреннего использования"""
    if not cards:
        return ""
    result = []
    for c in cards:
...
def build_message(game_num, player_cards, dealer_cards, p_score, d_score, state):
    p_hand_colored = format_cards_hand(player_cards)
    p_hand_raw = format_cards_raw(player_cards)
    d_hand_colored = format_cards_hand(dealer_cards)
    d_hand_raw = format_cards_raw(dealer_cards)
    total = p_score + d_score if dealer_cards else p_score
    
    if is_game_finished(state, player_cards, dealer_cards, p_score, d_score):
        if p_score > 21:
            return f"#N{game_num}. {p_score}({p_hand_colored}) - ✅{d_score}({d_hand_colored}) #T{total}"
        if d_score > 21:
            return f"#N{game_num}. ✅{p_score}({p_hand_colored}) - {d_score}({d_hand_colored}) #T{total}"
        if p_score == 21:
            return f"#N{game_num}. ✅{p_score}({p_hand_colored}) - {d_score}({d_hand_colored}) #T{total}"
    ...
    return f"#N{game_num}. {p_score}({p_hand_colored}) {arrow} {d_score}({d_hand_colored}) #T{total}"

def send_message(text):
    try:
        r = requests.post(API + "/sendMessage", json={"chat_id": CHAT_ID, "text": text, "parse_mode": "HTML"})
