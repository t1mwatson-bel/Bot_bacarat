...
# =====================================================================
# ALGORITHM: ПЕРВАЯ 10 → РАНГ ПЕРВОЙ КАРТЫ ИГРОКА
# =====================================================================

def get_rank_prediction(game):
    """
    Алгоритм "Первая 10 → ранг первой карты игрока".

    Триггер:
        - берём карты в порядке появления:
          сначала карты игрока, потом карты дилера
        - ищем ПЕРВУЮ десятку в этом порядке
        - если десятки нет — прогноза нет

    Отсев по тузу:
        - смотрим промежуток между первой картой игрока
          и найденной десяткой (не включая их самих)
        - если в промежутке есть туз — прогноз отсеивается
        - туз первой картой игрока — это цель, не отсев
        - туз ПОСЛЕ десятки — регулируется флагом
          REJECT_ACE_AFTER_TEN

    Целевая игра (догон 0):
        game_number + (индекс десятки) + 1
    ...
    # Ищем первую десятку в общем порядке.
    ten_index = None
    for idx, card in enumerate(all_cards):
        if normalize_rank(card.get("rank")) == "10":
            ten_index = idx
            break

    # Нет десятки — нет триггера.
    ...
    # Промежуток между первой картой игрока и десяткой.
    # all_cards[0] — первая карта игрока (она же цель).
    # all_cards[ten_index] — десятка.
    ...
    # Отсев: туз после десятки (если включён флаг).
...
def main():
    ...
    print(
        f"🚫 Отсев по тузу до десятки: ВКЛ",
        flush=True,
    )
    print(
